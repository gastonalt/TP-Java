...
public class UsuarioDao {
	...
	private static final String UPDATE_USUARIO_SQL = 
	    "UPDATE usuarios SET username = ?, password = ?, nombre = ?, apellido = ?, fecha_nacimiento = ?, direccion = ?, email = ?, localidad_id = ? WHERE id_usuario = ?;";
	...
	public Usuario usuario(int id_usuario) {
		...
		try (Connection connection = DatabaseUtils.getConnection();
			...
			while (rs.next()) {
				...
				usuario = new Usuario(id_usuario, username, password, nombre, apellido, fechaNacimiento, direccion, email, localidad);
			}
		} catch (SQLException e) {
			DatabaseUtils.printSQLException(e);
		}
